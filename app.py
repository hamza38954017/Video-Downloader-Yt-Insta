...
║                  Created by Dr. Hamza                        ║
║           Deploy on Render / Any Free Web Service            ║
╠══════════════════════════════════════════════════════════════╣
║  HOW THE BOT BYPASS WORKS:                                   ║
║  curl_cffi impersonates a real Chrome browser at the         ║
║  TLS fingerprint level — YouTube cannot tell the             ║
║  difference from a real user. This works even on             ║
║  blocked cloud IPs (Render, Heroku, AWS etc.)                ║
╠══════════════════════════════════════════════════════════════╣
...
def get_cookie_file():
    ...
    for p in candidates:
        if p and os.path.isfile(p) and os.path.getsize(p) > 0:
            print(f"[Cookie] ✅  {p}")
            return p
    print("[Cookie] ⚠️  No cookies.txt — using Chrome impersonation only.")
    return None

COOKIE_FILE = get_cookie_file()

# ═══════════════════════════════════════════════════════════════
# curl_cffi SESSION — reused for all HTTP requests
# Impersonates Chrome at the TLS/JA3 fingerprint level.
# YouTube, Cloudflare, Instagram all see a real Chrome browser.
# ═══════════════════════════════════════════════════════════════

def get_cffi_session(impersonate="chrome120"):
    """Return a curl_cffi session that looks like real Chrome."""
...
# Uses yt-dlp's built-in impersonate feature (backed by curl_cffi)
# ═══════════════════════════════════════════════════════════════
...
def make_yt_opts(strategy_args: dict,
    ...
    Build yt-dlp options with:
      - curl_cffi Chrome impersonation (bypasses TLS fingerprinting)
      - chosen YouTube player client (bypasses bot check)
...
def _build_format_lists(raw: dict):
    ...
    for f in sorted_fmts:
        h = f.get('height')
        if not h or h in seen_h: continue
        seen_h.add(h)
        ...
        mp4_formats.append({'quality': f'{h}p',
                            'format_id': fid, 'ext': 'mp4', 'filesize': sz})
    ...
    for f in sorted_fmts:
        h  = f.get('height')
        vc = f.get('vcodec', 'none')
        if not h or vc == 'none' or h in seen_mh: continue
        seen_mh.add(h)
        sz = f.get('filesize') or f.get('filesize_approx') or 0
...
# ═══════════════════════════════════════════════════════════════
# YOUTUBE — FETCH INFO
# Tries every combination of (player_client × impersonate_target)
# ═══════════════════════════════════════════════════════════════
...
def fetch_youtube_info(url):
    ...
    for imp_target in YT_IMPERSONATE_TARGETS:
        for idx, strategy in enumerate(YT_CLIENT_STRATEGIES):
            try:
                opts = make_yt_opts(strategy, imp_target,
                                    {'skip_download': True})
                with yt_dlp.YoutubeDL(opts) as ydl:
                    raw = ydl.extract_info(url, download=False)

                mp4_formats, mp3_formats, muted_formats = \
                    _build_format_lists(raw)
                ...
                result = {
                    ...
                    'channel_follower_count':
                                     raw.get('channel_follower_count', 0),
                    'categories':
                        ', '.join(raw.get('categories', [])[:3]),
                    'tags':
                        ', '.join((raw.get('tags', []) or [])[:5]),
                }
                print(f"[YT Info] ✅  impersonate={imp_target} "
                      f"client={strategy['player_client']} worked.")
    ...
    if last_error and is_bot_error(last_error):
        return {'error': (
            '⚠️ YouTube is still blocking this server IP. '
            'Add a fresh cookies.txt (exported while logged into YouTube) '
            'to the project folder and redeploy.'
        )}
    return {'error': (
        f'Could not fetch video info after all attempts. '
...
# ─────────────────────────────────────────────
# INSTAGRAM STREAM — uses curl_cffi for direct video fetch
# ─────────────────────────────────────────────
...
def _stream_instagram(url, fname, video_url=None):
    try:
        ...
        # Use curl_cffi to fetch — bypasses Instagram CDN restrictions
...
# ─────────────────────────────────────────────
# YOUTUBE STREAM — curl_cffi impersonation + multi-client fallback
# ─────────────────────────────────────────────
...
def _stream_youtube(url, format_id, fname, dl_type='mp4'):
    ...
    # Try every (impersonate × client) combination
    for imp_target in YT_IMPERSONATE_TARGETS:
        for idx, strategy in enumerate(YT_CLIENT_STRATEGIES):
            ...
            try:
                ...
                content_type = ('audio/mpeg' if file_ext == 'mp3'
                                else 'video/mp4')
                suffix       = '_muted' if is_muted else ''
                dl_name      = f'{fname}{suffix}.{file_ext}'

                print(f"[YT DL] ✅  impersonate={imp_target} "
                      f"client={strategy['player_client']}")
    ...
    return jsonify({
        'error': (f'Download failed on all strategies. '
                  f'{(last_err or "")[:200]}')
    }), 500

# ─────────────────────────────────────────────
# THUMBNAIL — downloaded via curl_cffi Chrome session
# ─────────────────────────────────────────────
...
def download_thumbnail():
    ...
    try:
        ...
        return Response(
            ...
            headers={
                'Content-Disposition': (
                    f'attachment; filename="{fname}_thumbnail.{ext}"'),
                'Content-Type': ctype,
            }
        )
    except Exception as e:
...
@app.route('/test-yt')
def test_yt():
    """Visit /test-yt in browser to run a live YouTube diagnostic."""
    ...
    return jsonify({
        ...
        'impersonate_targets': len(YT_IMPERSONATE_TARGETS),
        'client_strategies':   len(YT_CLIENT_STRATEGIES),
        'total_combinations': (len(YT_IMPERSONATE_TARGETS)
...
.hero h1{font-family:'Orbitron',sans-serif;font-size:clamp(26px,5.5vw,56px);font-weight:900;
...
.cookie-steps li .csn{background:var(--orange);color:#fff;width:20px;height:20px;border-radius:50%;
...
<main>
<div class="hero">
  <div class="h-badge"><span class="bdot"></span>Unlimited · Free · Chrome Bypass · Highest Quality</div>
  <h1>Download Any Video,<br><span class="g">Instantly &amp; Free.</span></h1>
  <p>The most powerful Instagram &amp; YouTube downloader — powered by Chrome TLS impersonation to bypass any IP block. MP4, MP3, Muted, Thumbnails.</p>
  <div class="stats">
    ...
    <div class="stat"><div class="sn">18</div><div class="sl">Bypass Combos</div></div>
...
<div class="mb-52">
  ...
  <div class="feat-grid">
    <div class="feat"><span class="fi">🛡️</span><div class="ft">Chrome TLS Bypass</div><div class="fd">Uses curl_cffi to impersonate real Chrome at the TLS fingerprint level — YouTube and Cloudflare cannot distinguish it from a real user.</div></div>
    <div class="feat"><span class="fi">🎬</span><div class="ft">MP4 (Video + Audio)</div><div class="fd">Full quality video with sound up to 4K UHD. Default always auto-picks the highest available resolution.</div></div>
    <div class="feat"><span class="fi">🎵</span><div class="ft">MP3 (Audio Only)</div><div class="fd">Extract pure high-quality MP3 from any YouTube video. Perfect for music, podcasts, and lectures.</div></div>
    <div class="feat"><span class="fi">🔇</span><div class="ft">Muted MP4 (No Sound)</div><div class="fd">Video-only track with no audio. Ideal for b-roll footage, video editing, and background visuals.</div></div>
    ...
    <div class="feat"><span class="fi">🔁</span><div class="ft">18 Bypass Combos</div><div class="fd">3 Chrome versions × 6 YouTube clients = 18 combinations tried automatically until one works.</div></div>
...
<div class="mb-52">
  ...
  <div class="steps-grid">
    <div class="step"><div class="snum">1</div><div class="stitle">Pick Platform</div><div class="sdesc">Tap Instagram or YouTube tab. Auto-detects from any pasted URL.</div></div>
    ...
    <div class="step"><div class="snum">3</div><div class="stitle">Fetch Details</div><div class="sdesc">Click Fetch to load title, uploader, date, views, likes, and all quality options.</div></div>
  ...
  <div class="tips">
    ...
    <ul>
      ...
      <li>Use <em>Muted MP4</em> for silent video editing b-roll footage.</li>
      <li>Visit <code>/test-yt</code> to run a live YouTube diagnostic on the server.</li>
      <li>Cookie pill turns green when cookies.txt is loaded and active.</li>
    </ul>
  </div>
</div>

<div class="cookie-box mb-52">
  <h3><i class="fas fa-cookie-bite"></i> YouTube Cookie Setup — Extra Layer for Age-Restricted Videos</h3>
  <ul class="cookie-steps">
    ...
    <li><div class="csn">5</div><div>Cookie pill turns <span style="color:var(--green);font-weight:700">green</span> when active. Visit <code>/test-yt</code> to verify everything works.</div></li>
  </ul>
</div>
...
<div class="faqs mb-52">
  ...
  <div class="faq" onclick="toggleFaq(this)">
    <div class="fq">How does the Chrome bypass work?<div class="fq-icon"><i class="fas fa-chevron-down"></i></div></div>
    <div class="fa-ans">This app uses <strong>curl_cffi</strong> to impersonate a real Chrome browser at the TLS/JA3 fingerprint level. When YouTube or Cloudflare check if the connection is from a real browser, the TLS fingerprint matches Chrome exactly — so they can't tell it apart from a real user, even on a cloud server IP.</div>
  </div>
  <div class="faq" onclick="toggleFaq(this)">
  ...
  <div class="faq" onclick="toggleFaq(this)">
    ...
    <div class="fa-ans">No — only public Instagram accounts and posts are supported. Private accounts require authentication which this tool does not collect, to respect user privacy and platform terms.</div>
  </div>
  <div class="faq" onclick="toggleFaq(this)">
    <div class="fq">What video quality can I download from YouTube?<div class="fq-icon"><i class="fas fa-chevron-down"></i></div></div>
    <div class="fa-ans">From 144p up to 4K UHD (2160p) depending on what the creator uploaded. The default <em>Best Quality (Auto)</em> automatically selects and merges the highest video and audio streams. ffmpeg is installed automatically by the provided render.yaml.</div>
  </div>
</div>
</main>
...
function setFtype(type){
  ...
  Object.entries(map).forEach(([k,id])=>{
    document.getElementById('ft'+id).className='ftype-tab'+(k===type?` on ${k}`:'');
  });
...
async function fetchInfo(){
  ...
  try{
    const r=await fetch('/fetch-info',{method:'POST',
  ...
  }catch(e){showAlert('Network error — check connection and retry.','err');}
...
function renderInfo(info){
  ...
  pb.textContent=info.platform;pb.className='plat-badge '+info.platform;
  ...
  document.getElementById('mgrid').innerHTML=rows.map(m=>
    `<div class="mi"><span class="ml">${m.l}</span><span class="mv">${m.v}</span></div>`).join('');
...
async function dlVideo(){
  ...
  try{
    const r=await fetch('/download',{method:'POST',
      headers:{'Content-Type':'application/json'},
      body:JSON.stringify({url:curInfo.url,format_id:fmt,platform:curInfo.platform,
        dl_type:curFtype,title:curInfo.title,video_url:curInfo.video_url||''})});
    ...
    clearInterval(tick);showAlert('Download failed: '+e.message,'err');prog.className='dl-prog';
  }finally{
...
async function dlThumb(){
  ...
  try{
    const r=await fetch('/download-thumbnail',{method:'POST',
      headers:{'Content-Type':'application/json'},
...
if __name__ == '__main__':
    ...
    print(f"🍪  Cookie  : {COOKIE_FILE or 'NOT FOUND (Chrome impersonation active)'}")
    print(f"🛡️  Bypass  : {len(YT_IMPERSONATE_TARGETS)} Chrome targets × "
          f"{len(YT_CLIENT_STRATEGIES)} YT clients = "
          f"{len(YT_IMPERSONATE_TARGETS)*len(YT_CLIENT_STRATEGIES)} combos")
