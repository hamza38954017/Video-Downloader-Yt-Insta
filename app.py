...
4. Rename file to: cookies.txt

LOCAL DEPLOYMENT:
   Place cookies.txt in the SAME FOLDER as app.py
   your_project/
     ├── app.py
     ├── cookies.txt   ← HERE
     ├── requirements.txt
...
def get_cookie_file():
    """
    Locate cookies.txt in priority order:
    ...
    for p in candidates:
        if p and os.path.isfile(p) and os.path.getsize(p) > 0:
            print(f"[Cookie] ✅  Found: {p}")
            return p
    print("[Cookie] ⚠️   No cookies.txt found — using client-bypass strategies only.")
    return None
...
# tv_embedded & ios bypass YouTube's bot/login check on cloud IPs.
# ═══════════════════════════════════════════════════════════════

YT_CLIENT_STRATEGIES = [
    # 1 — TV Embedded: no bot checks at all (best for cloud servers)
    {'player_client': ['tv_embedded']},
    # 2 — iOS mobile app endpoint: different auth flow, no challenges
    {'player_client': ['ios']},
    # 3 — Android VR: rarely flagged
    {'player_client': ['android_vr']},
    # 4 — Mobile web + Android combo
    {'player_client': ['mweb', 'android']},
    # 5 — Android Creator studio
    {'player_client': ['android_creator']},
    # 6 — Web creator (less restricted than plain web)
    {'player_client': ['web_creator', 'android']},
]


def make_yt_opts(strategy_args: dict, extra: dict = None) -> dict:
    """Build a yt-dlp options dict for a given client strategy."""
    opts = {
        'quiet': True,
        'no_warnings': True,
...
def is_bot_error(msg: str) -> bool:
    keywords = ('sign in', 'bot', 'login', 'verify', 'private',
                 'not available', 'confirm your age', 'age-restricted')
...
# ═══════════════════════════════════════════════════════════════
# YOUTUBE — FETCH INFO  (multi-strategy fallback)
# ═══════════════════════════════════════════════════════════════

def _build_format_lists(raw: dict):
    """Parse raw yt-dlp info dict into 3 format lists."""
    all_fmts    = raw.get('formats', [])
    sorted_fmts = sorted(all_fmts,
    ...
    mp3_formats = [
        {'quality': '🎵 Best Quality (Auto)',
         'format_id': 'bestaudio/best',            'ext': 'mp3', 'filesize': 0},
        {'quality': '🎵 High Quality (192 kbps)',
         'format_id': 'bestaudio[abr<=192]/best',  'ext': 'mp3', 'filesize': 0},
        {'quality': '🎵 Medium Quality (128 kbps)',
         'format_id': 'bestaudio[abr<=128]/best',  'ext': 'mp3', 'filesize': 0},
        {'quality': '🎵 Low Quality (96 kbps)',
         'format_id': 'bestaudio[abr<=96]/best',   'ext': 'mp3', 'filesize': 0},
    ]

    # ── Muted MP4 (video only) ───────────────────────────────
    muted_formats = [{
    ...
    for f in sorted_fmts:
        ...
        muted_formats.append({
            ...
            'format_id': (f'bestvideo[height={h}][ext=mp4]'
...
def fetch_youtube_info(url):
    ...
    for idx, strategy in enumerate(YT_CLIENT_STRATEGIES):
        try:
            opts = make_yt_opts(strategy, {'skip_download': True})
            ...
            print(f"[YT Info] ✅  Strategy {idx+1} "
                  f"({strategy['player_client']}) worked.")
            return {'success': True, 'info': result}

        except Exception as e:
            last_error = str(e)
            print(f"[YT Info] ❌  Strategy {idx+1} "
                  f"({strategy['player_client']}) failed: {last_error[:100]}")
            continue
    ...
    if last_error and is_bot_error(last_error):
        return {'error': (
            ...
            'Fix: place a valid cookies.txt (exported while logged into YouTube) '
            'in the project folder and redeploy. '
            'See the Cookie Setup guide on the page.'
        )}
    return {
        'error': (f'Could not fetch video info after '
                  f'{len(YT_CLIENT_STRATEGIES)} attempts. '
...
# ─────────────────────────────────────────────
# YOUTUBE STREAM  (multi-strategy fallback)
# ─────────────────────────────────────────────
...
def _stream_youtube(url, format_id, fname, dl_type='mp4'):
    ...
    for idx, strategy in enumerate(YT_CLIENT_STRATEGIES):
        ...
        try:
            ...
            last_err = str(e)
            print(f"[YT Download] ❌  Strategy {idx+1} "
                  f"({strategy['player_client']}) failed: {last_err[:100]}")
            cleanup_dir(tmpdir)
            continue

    return jsonify({
        'error': (f'Download failed on all {len(YT_CLIENT_STRATEGIES)} '
                  f'strategies. {last_err[:200] if last_err else ""}')
    }), 500
        ...
        return Response(
            ...
            headers={
                'Content-Disposition': (f'attachment; '
                                        f'filename="{fname}_thumbnail.{ext}"'),
                'Content-Type': ctype,
            }
        )
    except Exception as e:
...
@app.route('/test-yt')
def test_yt():
    """Visit /test-yt in browser to diagnose YouTube connectivity."""
    ...
    return jsonify({
        ...
        'strategies_tried': len(YT_CLIENT_STRATEGIES),
    })
...
.blob1{width:700px;height:700px;background:radial-gradient(circle,rgba(0,245,255,.2),transparent 65%);
  top:-280px;left:-200px;animation:bm1 24s ease-in-out infinite;}
.blob2{width:600px;height:600px;background:radial-gradient(circle,rgba(255,0,110,.17),transparent 65%);
  bottom:-180px;right:-180px;animation:bm2 30s ease-in-out infinite;}
.blob3{width:450px;height:450px;background:radial-gradient(circle,rgba(139,92,246,.13),transparent 65%);
  top:45%;left:45%;transform:translate(-50%,-50%);animation:bm3 20s ease-in-out infinite;}
...
.logo-icon{width:46px;height:46px;
  ...
  font-size:22px;animation:lpulse 3s ease-in-out infinite;flex-shrink:0;}
@keyframes lpulse{
  0%,100%{box-shadow:0 0 22px rgba(0,245,255,.45);}
  50%{box-shadow:0 0 55px rgba(0,245,255,.85),0 0 90px rgba(255,0,110,.3);}
}
.logo-text .name{
...
.logo-text .tag{font-size:9px;color:var(--sub);letter-spacing:2.5px;
  text-transform:uppercase;margin-top:1px;}
.h-right{display:flex;align-items:center;gap:18px;}
.credit .cl{font-size:9px;color:var(--sub);text-transform:uppercase;
...
.ck-pill{display:inline-flex;align-items:center;gap:6px;padding:5px 13px;
  border-radius:20px;font-size:11px;font-weight:600;letter-spacing:.5px;
...
.ck-dot{width:7px;height:7px;border-radius:50%;background:currentColor;
...
.h-badge{
  ...
  padding:7px 22px;font-size:11px;color:var(--cyan);
  letter-spacing:2px;text-transform:uppercase;margin-bottom:22px;
  animation:badge-glow 2.5s ease-in-out infinite;}
@keyframes badge-glow{
  0%,100%{border-color:rgba(0,245,255,.28);}
  50%{border-color:rgba(0,245,255,.75);box-shadow:0 0 22px rgba(0,245,255,.22);}
}
.bdot{width:6px;height:6px;background:var(--cyan);border-radius:50%;
...
.hero p{color:var(--sub);font-size:15px;max-width:540px;
...
.stat .sl{font-size:10px;color:var(--sub);text-transform:uppercase;
...
.ptab.on   {background:linear-gradient(135deg,var(--cyan),var(--purple));
  color:#fff;box-shadow:0 4px 24px rgba(0,245,255,.3);}
.ptab.on.yt{background:linear-gradient(135deg,#ff4444,#cc0000);
  box-shadow:0 4px 24px rgba(255,68,68,.35);}
...
/* ── LOADING BAR ── */
.lbar{height:3px;background:var(--inp);border-radius:2px;overflow:hidden;
  display:none;margin-bottom:20px;}
.lbar.on{display:block;}
.lbar-fill{height:100%;width:35%;
...
.btn-c  {background:linear-gradient(135deg,var(--cyan),var(--purple));color:#fff;
  box-shadow:0 4px 20px rgba(0,245,255,.22);}
...
.thumb-wrap img{width:185px;height:116px;object-fit:cover;
  border-radius:10px;border:1px solid var(--cb);}
...
.mgrid{display:grid;grid-template-columns:repeat(auto-fill,minmax(130px,1fr));
  gap:8px;margin-bottom:10px;}
.mi .ml{font-size:9px;color:var(--sub);text-transform:uppercase;
...
.ftype-lbl{font-size:11px;color:var(--sub);text-transform:uppercase;
  letter-spacing:2px;margin-bottom:10px;display:flex;align-items:center;gap:7px;}
...
.ftype-tab.on.mp4  {background:linear-gradient(135deg,#0ea5e9,#6366f1);
  color:#fff;box-shadow:0 4px 16px rgba(14,165,233,.35);}
.ftype-tab.on.mp3  {background:linear-gradient(135deg,var(--green),#00c97a);
  color:#07071a;box-shadow:0 4px 16px rgba(0,255,157,.3);}
.ftype-tab.on.muted{background:linear-gradient(135deg,#475569,#334155);
  color:#e2e8f0;box-shadow:0 4px 16px rgba(0,0,0,.4);}

/* ── QUALITY SELECT ── */
.fsec{margin-bottom:18px;display:none;}
.fsec.show{display:block;}
.slabel{font-size:11px;color:var(--sub);text-transform:uppercase;
...
.fsel{width:100%;background:var(--inp);border:1px solid var(--cb);border-radius:10px;
  padding:12px 38px 12px 16px;color:var(--text);
...
/* ── PROGRESS ── */
.dl-prog{display:none;background:var(--inp);border:1px solid var(--cb);
...
@keyframes pglow{
  0%,100%{box-shadow:0 0 10px rgba(0,245,255,.3);}
  50%{box-shadow:0 0 28px rgba(0,245,255,.75);}
}

/* ── SECTION HEADER ── */
.sec-h{margin-bottom:28px;}
.sec-title{
  font-family:'Orbitron',sans-serif;font-size:20px;font-weight:700;margin-bottom:6px;
  background:linear-gradient(90deg,var(--cyan),var(--pink));
  -webkit-background-clip:text;-webkit-text-fill-color:transparent;background-clip:text;}
...
.feat-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(200px,1fr));
  gap:14px;margin-bottom:54px;}
.feat{background:var(--card);border:1px solid var(--cb);border-radius:16px;
  padding:22px;backdrop-filter:blur(20px);transition:var(--trans);}
.feat:hover{transform:translateY(-5px);border-color:rgba(0,245,255,.38);
  box-shadow:0 14px 44px rgba(0,245,255,.12);}
...
/* ── STEPS ── */
.steps-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(175px,1fr));
...
.tips{background:linear-gradient(135deg,rgba(0,245,255,.05),rgba(255,0,110,.05));
  border:1px solid rgba(0,245,255,.2);border-radius:16px;
  padding:22px 26px;margin-bottom:52px;}
.tips-title{font-weight:700;font-size:14px;color:var(--cyan);
  margin-bottom:12px;display:flex;align-items:center;gap:8px;}
.tips ul{list-style:none;
  display:grid;grid-template-columns:repeat(auto-fit,minmax(230px,1fr));gap:9px;}
.tips ul li{font-size:13px;color:var(--sub);
  display:flex;align-items:flex-start;gap:8px;}
.tips ul li::before{content:'→';color:var(--cyan);flex-shrink:0;}

/* ── COOKIE BOX ── */
.cookie-box{
  background:linear-gradient(135deg,rgba(255,140,0,.07),rgba(255,68,68,.05));
  border:1px solid rgba(255,140,0,.3);border-radius:16px;
  padding:22px 26px;margin-bottom:52px;}
.cookie-box h3{color:var(--orange);font-size:15px;margin-bottom:14px;
  display:flex;align-items:center;gap:8px;}
.cookie-steps{list-style:none;display:flex;flex-direction:column;gap:10px;}
.cookie-steps li{font-size:13px;color:var(--sub);
...
.cookie-steps code{background:rgba(255,255,255,.08);padding:2px 7px;
  border-radius:5px;font-family:monospace;font-size:12px;color:var(--cyan);}
...
.fq{padding:18px 20px;cursor:pointer;
...
.fa-ans{max-height:0;overflow:hidden;
...
footer{text-align:center;padding:44px 20px;
  border-top:1px solid var(--cb);color:var(--sub);font-size:13px;}
...
<div class="hero">
  ...
  <p>The most powerful Instagram &amp; YouTube downloader — grab MP4, MP3 audio, muted video, and thumbnails in the best quality with zero limits.</p>
  <div class="stats">
...
<div class="card">
  <div class="alert" id="alertBox">
    <i class="fas fa-circle-info"></i><span id="alertMsg"></span>
  </div>
  ...
  <div class="ftype-wrap" id="ftypeWrap">
    ...
    <div class="ftype-tabs">
      <button class="ftype-tab on mp4" id="ftMP4" onclick="setFtype('mp4')">
        ...
        <small style="opacity:.65;font-size:10px">Video+Audio</small>
      </button>
      <button class="ftype-tab" id="ftMP3" onclick="setFtype('mp3')">
        <i class="fas fa-music"></i> MP3
        <small style="opacity:.65;font-size:10px">Audio Only</small>
      </button>
      <button class="ftype-tab" id="ftMuted" onclick="setFtype('muted')">
        <i class="fas fa-volume-xmark"></i> Muted MP4
        <small style="opacity:.65;font-size:10px">No Sound</small>
      </button>
    </div>
  </div>
  ...
  <div class="abts" id="abts">
    <button class="btn btn-g" id="dlBtn" onclick="dlVideo()">
      ...
      <span id="dlBtnTxt">Download MP4</span>
    </button>
...
<div class="mb-52">
  ...
  <div class="feat-grid">
    <div class="feat"><span class="fi">🎬</span>
      <div class="ft">MP4 (Video + Audio)</div>
      <div class="fd">Full quality video with sound up to 4K UHD. Default always auto-picks the highest available resolution.</div>
    </div>
    <div class="feat"><span class="fi">🎵</span>
      <div class="ft">MP3 (Audio Only)</div>
      <div class="fd">Extract pure high-quality MP3 from any YouTube video. Great for music, podcasts, and lectures.</div>
    </div>
    <div class="feat"><span class="fi">🔇</span>
      <div class="ft">Muted MP4 (No Sound)</div>
      <div class="fd">Video track only with no audio. Ideal for b-roll footage, video editing, and background visuals.</div>
    </div>
    <div class="feat"><span class="fi">📸</span>
      <div class="ft">Instagram Posts & Reels</div>
      <div class="fd">Download photos, videos, carousels, and Reels from any public Instagram account at original quality.</div>
    </div>
    <div class="feat"><span class="fi">🖼️</span>
      <div class="ft">Thumbnail Downloader</div>
      <div class="fd">Save the highest-resolution thumbnail from any Instagram post or YouTube video in one click.</div>
    </div>
    <div class="feat"><span class="fi">📊</span>
      <div class="ft">Full Video Details</div>
      <div class="fd">See uploader, date, views, likes, subscribers, duration, and description before downloading.</div>
    </div>
    <div class="feat"><span class="fi">🛡️</span>
      <div class="ft">6-Strategy Bot Bypass</div>
      <div class="fd">Automatically tries 6 different YouTube clients (TV, iOS, VR, mweb…) until one works — no manual action needed.</div>
    </div>
    <div class="feat"><span class="fi">🌙</span>
      <div class="ft">Day / Night Mode</div>
      <div class="fd">Fully themed dark and light mode with your preference saved automatically between visits.</div>
    </div>
  </div>
</div>
...
<div class="mb-52">
  ...
  <div class="steps-grid">
    <div class="step">
      <div class="snum">1</div><div class="stitle">Pick Platform</div>
      <div class="sdesc">Tap the Instagram or YouTube tab. The platform also auto-detects when you paste any URL.</div>
    </div>
    <div class="step">
      <div class="snum">2</div><div class="stitle">Paste URL</div>
      <div class="sdesc">Copy the video link from Instagram or YouTube and paste it into the input field above.</div>
    </div>
    <div class="step">
      <div class="snum">3</div><div class="stitle">Fetch Details</div>
      <div class="sdesc">Click Fetch to load title, uploader, date, views, likes, and all available quality formats.</div>
    </div>
    <div class="step">
      <div class="snum">4</div><div class="stitle">Choose Type</div>
      <div class="sdesc">Select <strong>MP4</strong>, <strong>MP3</strong>, or <strong>Muted MP4</strong> then pick quality from the dropdown.</div>
    </div>
    <div class="step">
      <div class="snum">5</div><div class="stitle">Download</div>
      <div class="sdesc">Hit the Download button and the file saves directly to your device. Done!</div>
    </div>
  </div>
  <div class="tips">
    ...
    <ul>
      ...
      <li>Use <em>Muted MP4</em> for clean b-roll and silent video editing footage.</li>
      <li>Instagram Reels, IGTV, and carousel posts are fully supported.</li>
      <li>If YouTube still fails, add a fresh <code>cookies.txt</code> — see guide below.</li>
    </ul>
  </div>
</div>
...
<div class="cookie-box mb-52">
  ...
  <ul class="cookie-steps">
    <li>
      <div class="csn">1</div>
      <div>Install <strong>"Get cookies.txt LOCALLY"</strong> extension on Chrome or Firefox. Log into YouTube first.</div>
    </li>
    <li>
      <div class="csn">2</div>
      <div>Go to <strong>youtube.com</strong>, click the extension icon, and export <code>cookies.txt</code> (Netscape format — do not edit the file).</div>
    </li>
    <li>
      <div class="csn">3</div>
      <div><strong>Local / VPS:</strong> Rename to <code>cookies.txt</code> and place it in the same folder as <code>app.py</code>. Restart the server.</div>
    </li>
    <li>
      <div class="csn">4</div>
      <div><strong>Render cloud:</strong> Service → <em>Settings → Secret Files</em> → path <code>/etc/secrets/cookies.txt</code> → paste content → Redeploy.</div>
    </li>
    <li>
      <div class="csn">5</div>
      <div>The 🍪 pill in the header turns <span style="color:var(--green);font-weight:700">green</span> when the cookie is active. Refresh to verify. Also visit <code>/test-yt</code> to run a live diagnostic.</div>
    </li>
  </ul>
</div>
...
<div class="faqs mb-52">
  ...
  <div class="faq" onclick="toggleFaq(this)">
    <div class="fq">Is Social Crazy Dr. Dev completely free to use?
      <div class="fq-icon"><i class="fas fa-chevron-down"></i></div></div>
    <div class="fa-ans">Yes — 100% free with no limits. No registration, no subscription, no daily cap. Download as many videos as you want from Instagram and YouTube at any time, forever.</div>
  </div>

  <div class="faq" onclick="toggleFaq(this)">
    <div class="fq">What is the difference between MP4, MP3, and Muted MP4?
      <div class="fq-icon"><i class="fas fa-chevron-down"></i></div></div>
    <div class="fa-ans"><strong>MP4</strong> downloads a full video with both video and audio merged. <strong>MP3</strong> extracts only the audio and saves it as a high-quality MP3 file — great for music and podcasts. <strong>Muted MP4</strong> saves only the video track with zero audio — perfect for video editors who want clean b-roll footage without any sound.</div>
  </div>

  <div class="faq" onclick="toggleFaq(this)">
    <div class="fq">Why does YouTube ask me to sign in or detect a bot?
      <div class="fq-icon"><i class="fas fa-chevron-down"></i></div></div>
    <div class="fa-ans">YouTube flags cloud server IPs (Render, AWS, Heroku etc.) as bots. This app automatically tries 6 different YouTube client bypass strategies (TV Embedded, iOS, Android VR, mweb…) to get around this. If all fail, adding a <code>cookies.txt</code> file exported while logged into YouTube resolves it permanently.</div>
  </div>

  <div class="faq" onclick="toggleFaq(this)">
    <div class="fq">Can I download private Instagram posts?
      <div class="fq-icon"><i class="fas fa-chevron-down"></i></div></div>
    <div class="fa-ans">No — only public Instagram accounts and posts are supported. Private accounts require authentication, which this tool intentionally does not collect to respect user privacy and platform terms of service.</div>
  </div>

  <div class="faq" onclick="toggleFaq(this)">
    <div class="fq">What video quality can I download from YouTube?
      <div class="fq-icon"><i class="fas fa-chevron-down"></i></div></div>
    <div class="fa-ans">From 144p all the way up to 4K UHD (2160p), depending on what the creator uploaded. The default <em>Best Quality (Auto)</em> automatically selects and merges the highest video and audio streams using ffmpeg. You can also pick any specific resolution, or go audio-only with MP3. ffmpeg is installed automatically by the provided <code>render.yaml</code>.</div>
  </div>
</div>
...
<footer>
  ...
  <p style="margin-top:12px;font-size:11px;opacity:.4">
    For personal use only · Respect copyright and creator rights ·
...
async function checkCookie() {
  try {
    ...
    if (d.cookie_loaded) {
      ...
      pill.title = 'No cookies.txt — using client-bypass strategies';
    }
  } catch(e) {}
}
checkCookie();
...
function switchTab(p) {
  curPlatform = p;
  document.getElementById('tabIG').className =
    'ptab' + (p === 'instagram' ? ' on' : '');
  document.getElementById('tabYT').className =
    'ptab' + (p === 'youtube'   ? ' on yt' : '');
...
function setFtype(type) {
  ...
  Object.entries(map).forEach(([k, id]) => {
    document.getElementById('ft' + id).className =
      'ftype-tab' + (k === type ? ` on ${k}` : '');
  });
  if (curInfo) {
...
function renderInfo(info) {
  ...
  img.onerror = () =>
    img.src = 'https://placehold.co/185x116/07071a/00f5ff?text=No+Thumb';
  ...
  const rows = [];
  if (info.uploader)
    rows.push({l:'Uploaded By', v:'@' + info.uploader});
  if (info.uploader_full && info.uploader_full !== info.uploader)
    rows.push({l:'Full Name',   v: info.uploader_full});
  if (info.date)
    rows.push({l:'Date',        v: info.date});
  if (info.view_count)
    rows.push({l:'Views',       v: fmtN(info.view_count)});
  if (info.like_count || info.likes)
    rows.push({l:'Likes',       v: fmtN(info.like_count || info.likes)});
  if (info.duration)
    rows.push({l:'Duration',    v: fmtDur(info.duration)});
  if (info.comment_count || info.comments)
    rows.push({l:'Comments',    v: fmtN(info.comment_count || info.comments)});
  if (info.channel_follower_count)
    rows.push({l:'Subscribers', v: fmtN(info.channel_follower_count)});

  document.getElementById('mgrid').innerHTML = rows.map(m =>
    `<div class="mi">
  ...
  document.getElementById('ftypeWrap').className =
    isYT ? 'ftype-wrap show' : 'ftype-wrap';
  if (isYT) {
    ['MP4','MP3','Muted'].forEach(t =>
      document.getElementById('ft' + t).className = 'ftype-tab');
...
async function dlVideo() {
  ...
  const lbls = {
    mp4:   'Downloading MP4…',
    mp3:   'Extracting MP3…',
    muted: 'Downloading Muted…',
  };
  const tick = setInterval(() => {
    pct = Math.min(pct + Math.random() * 2.1, 87);
    pfill.style.width = pct + '%';
    plbl.textContent  = (lbls[curFtype] || 'Downloading…') +
                        ' ' + Math.round(pct) + '%';
  }, 280);
  ...
  try {
    ...
    triggerDownload(blob,
      `${fname}${sfxM[curFtype]}.${extM[curFtype]}`);
...
async function dlThumb() {
  if (!curInfo || !curInfo.thumbnail) {
    showAlert('No thumbnail available.', 'err'); return;
  }
  ...
  try {
    const r = await fetch('/download-thumbnail', {
      ...
      headers: {'Content-Type': 'application/json'},
      body: JSON.stringify({
        thumbnail_url: curInfo.thumbnail,
        filename: sfn(curInfo.title || 'thumbnail'),
      }),
    });
    if (!r.ok) throw new Error('Failed');
    triggerDownload(await r.blob(),
      sfn(curInfo.title || 'thumbnail') + '_thumbnail.jpg');
...
if __name__ == '__main__':
    ...
    print(f"🍪  Cookie : {COOKIE_FILE or 'NOT FOUND (using client-bypass strategies)'}")
    print(f"🎯  Strategies: {len(YT_CLIENT_STRATEGIES)} YouTube clients configured")
